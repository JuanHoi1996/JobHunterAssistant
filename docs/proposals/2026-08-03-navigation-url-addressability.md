...
> **状态：已确认（2026-08-03）。** 产品负责人已确认采用选项 C（URL 可寻址的客户端路由），并写入 `docs/DECISIONS.md`。
>
> **实现不在本文档 PR 中。** 阶段 1 路由实现须另开独立 PR（仅路由 / 刷新恢复 / 前进后退 / 新标签 / 无效岗位兜底）。
...
## 建议写入 DECISIONS 的文案（已写入）

已确认决策见 `docs/DECISIONS.md`「导航与路由」一节。

## 确认结果与下一步

产品负责人已确认：

1. **选项 C**：URL 可寻址的客户端路由；不重写成古典 MPA。
2. **阶段 1 路由表固定为**：
   - `/jobs`：岗位列表
   - `/jobs/[jobId]`：岗位工作区
   - `/resumes`：简历中心
   - 岗位内页签使用 `?tab=resume` 等查询参数
3. **实现**：另开独立的「阶段 1 路由实现」PR；不与本提案文档 PR 混写代码。

---

## 附录：贡献者立场（可忽略，不影响决策表）

贡献者个人更习惯 MPA 心智，但在本仓约束下，认为「URL 可寻址的客户端工作台」才是同时满足浏览器契约与本地隐私/编辑器形态的最小正确架构。
